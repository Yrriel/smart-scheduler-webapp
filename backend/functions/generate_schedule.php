...
$faculty = $conn->query("
    ...
    FROM manage_faculty WHERE current_status = 'Active'
")->fetch_all(MYSQLI_ASSOC);
...
- Do NOT modify or rebalance previously scheduled sections.
- Use occupied_slots as immutable, already-finalized schedules.
...
# FACULTY RULES (STRICT)
- Faculty may ONLY teach subjects listed in faculty_subjects[faculty]
- Faculty may ONLY be scheduled on days listed in faculty_days[faculty]
- Faculty weekly hours MUST NOT exceed total_hours_per_week
...
# EXISTING OCCUPIED SLOTS (HARD CONSTRAINT)
You will receive an array called "occupied_slots".
...
You MUST treat these as ALREADY BOOKED.

Before assigning ANY schedule entry:
- The new entry MUST NOT overlap in time/day with:
  - Any occupied room
  - Any occupied faculty
  - Any occupied section

If a conflict exists:
- Find the NEXT available time slot
- Or move to a different day
- Or change room if allowed

DO NOT reuse any occupied time ranges.
...
foreach ($sections as $section) {
    ...
    file_put_contents(
        $tempFile,
        json_encode(['occupied' => $occupiedSlots], JSON_PRETTY_PRINT)
    );
