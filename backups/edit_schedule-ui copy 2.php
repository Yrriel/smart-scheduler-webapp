...
<aside class="filter-sidebar">
  ...
  <button class="view-tab" data-view="faculty">
    <svg viewBox="0 0 24 24" fill="currentColor">
  ...
  <button class="view-tab" data-view="section">
    <svg viewBox="0 0 24 24" fill="currentColor">
  ...
  <button class="view-tab" data-view="room">
    <svg viewBox="0 0 24 24" fill="currentColor">
...
function validateSchedules() {
  ...
  for(let i=0;i<visible.length;i++){
    for(let j=i+1;j<visible.length;j++){
      const a=visible[i].dataset,b=visible[j].dataset;
      if(!overlap(
        timeToMinutes(a.start),timeToMinutes(a.end),
        timeToMinutes(b.start),timeToMinutes(b.end)
      ))continue;
      if(a.room===b.room)errors.push(`Room conflict (${a.room})`);
      if(a.faculty===b.faculty)errors.push(`Faculty conflict (${a.faculty})`);
    }
  }
  alert(errors.length
    ? `❌ Conflicts:\n\n${[...new Set(errors)].join('\n')}`
...
events.forEach(card=>{
  ...
  card.onmousedown=e=>{
    ...
    document.onmouseup=ev=>{
      ...
      card.style.transform='';
      card.style.gridColumnStart=Math.max(2,oc+Math.round((ev.clientX-sx)/COL_WIDTH));
      card.style.gridRowStart=Math.max(2,or+Math.round((ev.clientY-sy)/CELL_HEIGHT));
      card.style.gridRowEnd=parseInt(card.style.gridRowStart)+span;
