...
file_put_contents(
    ...
    foreach ($schedules as $row) {
        if (!fitsAvailableSlot($row, $availableSlotsByRoom)
            ...
            $aiMistakes[] = [
                ...
                'reason'  => 'Outside available slot or overlap'
            ];
            continue;
        }
        $finalSchedules[] = $row;
    ...
    //save mistakes in ai_mistakes.json for data
    file_put_contents(
    __DIR__ . '/ai_mistakes.json',
    json_encode(['mistakes'=>$aiMistakes], JSON_PRETTY_PRINT)
);
