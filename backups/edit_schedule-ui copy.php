...
function applyFilters() {
  allEvents.forEach(ev => {
    const matchDay =
      !filterDay.value || ev.dataset.day === filterDay.value;

    const matchRoom =
      !filterRoom.value || ev.dataset.room === filterRoom.value;
      
    const matchFaculty =
    !filterFaculty.value ||
    ev.dataset.faculty === filterFaculty.value;

    const matchSection =
    !filterSection.value ||
    ev.querySelector('.subject')?.nextElementSibling?.textContent === filterSection.value;


    ev.classList.toggle(
      'hidden',
      !(matchDay && matchRoom && matchFaculty && matchSection)
    );
  });
}

[filterDay, filterRoom, filterFaculty, filterSection]
...
<script>
const eventsData = [...document.querySelectorAll('.event')];

function uniqueValues(attr) {
  return [...new Set(eventsData.map(e => e.dataset[attr]).filter(Boolean))].sort();
}

function buildDropdown(inputId, menuId, values) {
  const input = document.getElementById(inputId);
  const menu  = document.getElementById(menuId);

  menu.innerHTML = `
    <input type="text" placeholder="Search..." />
    ${values.map(v => `<div class="dropdown-item">${v}</div>`).join('')}
  `;
...
// Build dropdowns
buildDropdown('filterFaculty', 'facultyMenu', uniqueValues('faculty'));

buildDropdown(
  'filterSection',
  'sectionMenu',
  [...new Set(eventsData.map(e =>
    e.querySelector('.subject')?.nextElementSibling?.textContent
  ).filter(Boolean))].sort()
);
