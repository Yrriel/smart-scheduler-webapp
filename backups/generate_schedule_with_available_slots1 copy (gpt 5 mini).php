...
$BATCH_SIZE   = 3;    // fewer items per request
$MAX_RETRIES  = 1;
...
function callOpenAIOnce($client, array $payload) {
    ...
    // 🔒 Ensure "json" keyword exists when using json_object
    if (
        isset($payload['response_format']) &&
        ($payload['response_format']['type'] ?? '') === 'json_object'
    ) {
        if (
            stripos($system, 'json') === false &&
            stripos($user, 'json') === false
        ) {
            // inject safely into system prompt
    ...
    return $client->responses()->create([
        ...
        'text' => [
            'format' => $payload['response_format'] ?? ['type' => 'text']
        ],
    ]);
}
...
$client = OpenAI::factory()
    ...
    /* =========================================================
   OPENAI SANITY TEST (RUN ONCE)
   ========================================================= */

try {
    $test = $client->responses()->create([
        'model' => 'gpt-5-mini',
        'input' => 'Return valid JSON only: {"ok": true}',
        'text' => [
            'format' => [
                'type' => 'json_object'
            ]
    ...
    file_put_contents(
        __DIR__ . '/generation_debug.log',
        date('c') . " | Sanity test OK: " . json_encode($test) . PHP_EOL,
        FILE_APPEND
    );
} catch (Throwable $e) {
    file_put_contents(
        __DIR__ . '/generation_debug.log',
        date('c') . " | Sanity test FAILED: " . $e->getMessage() . PHP_EOL,
        FILE_APPEND
    );
    exit; // ⛔ STOP if even this fails
}
...
OUTPUT RULE:
- Return schedule entries ONLY when all rules can be satisfied
- Do NOT invent or force placements
- Omitted assignments will be retried later
...
- If a room is occupied at a time, you MAY keep the same time by choosing a DIFFERENT available room
- You MUST change the time ONLY if NO room is available at that time
...
file_put_contents(
    ...
    $payload = [
        ...
        'max_tokens' => 3000,
        'messages'=>[
            ['role'=>'system','content'=>$promptBatch],
    ...
    $response = callOpenAIOnce($client, $payload);
        ...
        file_put_contents(
            __DIR__ . '/generation_debug.log',
            date('c') . " | OPENAI ERROR: " . $e->getMessage() . PHP_EOL,
            FILE_APPEND
        );
        sleep($BATCH_DELAY);
        continue;
    ...
    if (($response['status'] ?? '') === 'incomplete') {
        file_put_contents(
            __DIR__ . '/generation_debug.log',
            date('c') . " | BATCH INCOMPLETE: "
            . json_encode($response['incompleteDetails'] ?? []) . PHP_EOL,
            FILE_APPEND
        );
    }
    // $schedules = json_decode($response['output_text'], true)['schedules'] ?? [];
    $text = '';
    ...
    file_put_contents(
        __DIR__ . '/generation_debug.log',
        date('c') . " | AI returned EMPTY output" . PHP_EOL,
        FILE_APPEND
    );
    // continue;
    exit;
    ...
    $schedules = json_decode($text, true)['schedules'] ?? [];

    if (count($schedules) !== count($chunk)) {
        file_put_contents(
            __DIR__ . '/generation_debug.log',
            date('c') . " | INVALID COUNT (batch $chunkIndex): expected "
            . count($chunk) . ", got " . count($schedules) . PHP_EOL,
            FILE_APPEND
        );
...
if (!empty($retryAssignments)) {
    ...
    $retryResponse = callOpenAIOnce($client, $retryPayload);
        ...
        file_put_contents(
            __DIR__ . '/generation_debug.log',
            date('c') . " | RETRY OPENAI ERROR: " . $e->getMessage() . PHP_EOL,
            FILE_APPEND
        );

        // ⛔ Abort retry phase cleanly
        $retryResponse = null;
    }
    ...
    if ($retryResponse === null) {
        ...
        // foreach ($retryResponse['output'] ?? [] as $item) {
        //     if ($item['type'] === 'message') {
        //         $text .= $item['text'];
        //     }
        // }
        $text = trim($retryResponse['outputText'] ?? '');


        if ($text === '') {
            file_put_contents(
                __DIR__ . '/generation_debug.log',
                date('c') . " | RETRY AI returned EMPTY output" . PHP_EOL,
                FILE_APPEND
            );
            $retrySchedules = [];
        }
        ...
        $retrySchedules = json_decode($text, true)['schedules'] ?? [];
    }

    if (empty($retrySchedules)) {
        foreach ($retryAssignments as $a) {
            $aiMistakes[] = [
                'section'      => $a['section'],
                'subject'      => $a['subject'],
    ...
    /* 🔒 RETRY COUNT GUARD — PUT IT HERE */
    if (count($retrySchedules) > count($retryAssignments)) {
        file_put_contents(
            __DIR__ . '/generation_debug.log',
            date('c') . " | RETRY INVALID COUNT: expected <= "
            . count($retryAssignments) . ", got " . count($retrySchedules) . PHP_EOL,
            FILE_APPEND
        );
        // abort retry safely
        $retrySchedules = [];
