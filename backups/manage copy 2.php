                    ...
                    <tr>
                        ...
                        <td>
                            <button 
                                ...
                                data-students="<?= htmlspecialchars($row['total_students']) ?>"
                                data-units="<?= htmlspecialchars($row['total_units']) ?>"
                                data-hours="<?= htmlspecialchars($row['total_hours_per_week']) ?>"
                            >
                                <svg viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <tr>
                        ...
                        <td>
                            <button 
                                ...
                                data-capacity="<?= htmlspecialchars($row['room_capacity']) ?>"
                            >
                                <svg viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>
                                </svg>
    ...
    <div class="modal-backdrop hidden" id="addCourseModal">
        ...
        <form method="post" action="add_course.php">
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>Type</label>
                    <input type="text" name="type" placeholder="e.g.,College, Shs">
                </div>
                <div class="form-group">
                    <label>Status</label>
                    <select name="status">
        ...
        <form method="post" action="add_faculty.php">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="employment_type" placeholder="e.g.,Full-time">
                </div>
                <div class="form-group">
                    <label>Status</label>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    editButtons.forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            selectedSubjects = btn.dataset.subjects ? btn.dataset.subjects.split(",") : [];
            updateSubjectTags();
    ...
    // Prevent clicks inside suggestion box from closing modal
    suggestionBox.addEventListener("mousedown", e => e.stopPropagation());

    // ================= SUBJECT SEARCH =================
    subjectSearch.addEventListener("input", () => {
        const input = subjectSearch.value.trim();
        ...
        fetch("search_subjects.php?q=" + encodeURIComponent(input))
            .then(res => res.json())
            .then(data => {
                ...
                suggestionBox.innerHTML = data.map(s =>
                    `<div class='subject-option' data-subj='${s.subject}'>
                ...
                // Add mousedown listener to prevent losing focus
                document.querySelectorAll(".subject-option").forEach(opt => {
                    opt.addEventListener("mousedown", (event) => {
                        event.stopPropagation(); // Prevent modal click interference

                        const subj = opt.dataset.subj;
    ...
    // Hide box when input loses focus (AFTER click is processed)
    ...
    function updateSubjectTags() {
        ...
        selectedSubjects.forEach(s => {
            ...
            tag.innerHTML = `${s} <span class="remove-tag" data-subj="${s}">×</span>`;
        ...
        document.querySelectorAll(".remove-tag").forEach(btn => {
            btn.addEventListener("click", () => {
                selectedSubjects = selectedSubjects.filter(sub => sub !== btn.dataset.subj);
                updateSubjectTags();
            });
        });
    }
...
//             const tag = document.createElement("div");
//             tag.classList.add("subject-tag");
//             tag.innerHTML = `${sub} <span class="remove-tag" data-subj="${sub}">×</span>`;
//             selectedContainer.appendChild(tag);
//         });
...
//         document.querySelectorAll(".remove-tag").forEach(btn => {
//             btn.addEventListener("click", () => {
//                 selectedSubjects = selectedSubjects.filter(s => s !== btn.dataset.subj);
//                 updateSubjectTags();
//             });
//         });
//     }
...
//             fetch("get_faculty_subjects.php?faculty_name=" + encodeURIComponent(btn.dataset.name))
//                 .then(res => res.json())
//                 .then(data => {
...
//     // SUBJECT SEARCH (backend suggestions)
//     subjectSearch.addEventListener("input", () => {
//         const input = subjectSearch.value.trim();
...
//     // Subject Search (NOW USING BACKEND)
//     subjectSearch.addEventListener("input", () => {
//         const input = subjectSearch.value.trim();
...
//     subjectSearch.addEventListener("input", () => {
//         const input = subjectSearch.value.toLowerCase();
//         if (!input) {
//             suggestionBox.classList.add("hidden");
//             return;
//         }

//         // Static subjects list shown as example
//         const suggestions = allSubjects.filter(sub => sub.toLowerCase().includes(input));
    

//         suggestionBox.innerHTML = suggestions.map(s => 
//             `<div class='subject-option' data-subj='${s}'>${s}</div>`
//         ).join("");
...
// });
        ...
        // document.addEventListener("DOMContentLoaded", () => {
        //     const modalFaculty = document.getElementById("editFacultyModal");
        //     const cancelFaculty = document.getElementById("btncancelEdit-faculty");
        //     const editButtonsFaculty = document.querySelectorAll(".edit-btn-faculty");

        //     editButtonsFaculty.forEach(button => {
        //         button.addEventListener("click", () => {
        //             document.getElementById("edit_faculty_id").value = button.dataset.id;
        //             document.getElementById("edit_faculty_name").value = button.dataset.name;
        //             document.getElementById("edit_employment_type").value = button.dataset.employment;
        //             document.getElementById("edit_current_status").value = button.dataset.status;
        //             document.getElementById("edit_total_units").value = button.dataset.units;
        //             document.getElementById("edit_total_hours").value = button.dataset.hours;

        //             modalFaculty.classList.remove("hidden");
        //         });
        //     });

        //     cancelFaculty.addEventListener("click", () => {
        //         modalFaculty.classList.add("hidden");
        //     });
        // });
